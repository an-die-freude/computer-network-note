...
​		﹡数据平面即==路由器的功能==，用于从路由器的输入链路向输出链路转发分组，包括传统的==IP转发==和==通用转发==。IP转发仅根据IP转发，通用转发根据IP和其他因素转发。
...
​		若有$N$个输入端口和$N$个输出端口且输入线路与输入线路速度相同，都为$R_{line}(packet/s)$。此外，所有分组具有相同的固定长度且以同步的方式到达输入端口，即所有链路发送/接收分组的时间相等。用$R_{switch}$表示分组从输入端口传输到输出端口的速度。

###### 4.1.3.1 输入队列排队

​		假设采用纵横式交换且所有链路速度相同，从输入链路接收分组与分组从输入端口传输到给的输出端口所需时间相同，此外，分组采用FCFS方式。
