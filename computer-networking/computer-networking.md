...
​		UDP存在三个重大缺陷，首先，由于服务器和客户端之间的可用带宽不可预测且不断变化，因此恒定速率UDP流可能不能保证不间断的播放。其次，需要RTSP服务器这样的媒体控制服务器来为每个正在进行的客户端会话处理客户端到服务器的交互请求并跟踪客户端状态，这增加了部署大型按序视频系统的成本和复杂性。最后，许多防火墙配置为阻止UDP流量。
