...
​		当WLAN中的站点(无线主机或AP)发送帧时，由于各种原因，帧可能无法完整地达到目的地。为了处理这种不可忽略的故障，802.211 MAC协议使用了**链路层确认**。当接收端收到通过CRC的帧时，它会等待**短帧间间隔**后响应一个确认帧。若发送端在给定时间内未收到确认，则认为发送了错误并重新发送帧，在给定次数的重传后依然未收到确认则丢弃该帧。
...
​		CSMA/CA中，当某个站点有帧待发送时，它先侦听信道，当信道空闲时，它会等待**分布式帧间间隔**，若在此期间未侦听到信号就发送帧，否则该站点像[CSMA/CD](#6.2.2.4 CSMA/CD)使用二进制指数退避算法来计算退避时间并启动**退避计时器**。若在退避时间内侦听到信号则暂停计时器，然后会重新等待到信道空闲并在下一个DIFS后再次启动计时器(计时器时间是剩下的时间)，计时器为0时发送帧。
...
​		为了避免隐藏终端导致的问题，802.11协议允许无线主机使用短**请求发送**控制帧和短**允许发送**控制帧来预约信道的访问。当发送端有数据帧要发送时，可以先向AP发送一个RTS帧来指示传输数据帧和ACK帧所需的总时间。当AP收到RTS帧后通过广播CTS来响应，这样既给了发送端明确的发送权限，也指示了其他无线主机在预约时间内不要发送。

![collision_avoidance_using_rts_and_cts_frames](img/collision_avoidance_using_rts_and_cts_frames.png)

​		尽管RTS和CTS帧有助于减少碰撞，但也引入了延迟并消耗信道资源，因此，RTS和CTS帧仅用于长数据帧的传输通道预约。实际上可以为每个无线站点设置RTS阈值，当帧的长度超过阈值时才是RTS/CTS帧。很多无线站点的默认阈值大于最大帧长度，故所有发送的数据帧都会跳过RTS/CTS帧。
...
| to AP            | 1        | 值为1时表示帧发送到AP                               |
| from AP          | 1        | 值为1时表示帧来自于AP                               |
| more flag        | 1        | [类似于IPv4数据报中的分片标志位](#4.2.1 IPv4数据报) |
| retry            | 1        | 值为1时表示需要重传                                 |
...
​		持续器用于记载网络分配向量的值。当最后一位为0时用于设置NAV。站点必须监视该字段并更新。
