...
​		每个数据中心都有**数据中心网络**，将其主机互联并将数据中心与因特网互联。数据中心网络的关键需求是计算和服务放置的灵活。
...
​		数据中心网络支持两种流量：外部客户端和内部主机之间的流量以及内部主机之间的流量。为了处理外部客户端和内部主机之间的流量，数据中心网络包括一个或多个**边界路由器**用于将数据中心网络连接到因特网。所以数据中心网络需要将所有主机互联并将机架与边界路由器连接。
...
​		**WiFi丛林**指无线主机能从两个或多个AP接收到足够强的信号的任何物理位置。无线主机需要与AP**关联**才能加入AP所属子网并访问因特网，关联表示无线主机与AP建立一条虚拟链路。
