...
​		**地址解析协议**用于网络层地址与链路层地址之间的转换。ARP仅解析同一子网下的主机和路由器接口的IP地址。
...
​		ARP分组包括16位**硬件地址类型**、16位**协议类型**、8位**硬件地址长度**、8位**协议长度**、16位**操作码**、48位**发送端MAC地址**、32位**发送端IP地址**、48位**目的MAC地址**、32位**目的IP地址**。

​		﹡硬件地址类型为1时表示以太网。

​		﹡协议类型为0x0080时表示IP。

​		﹡操作码为1时表示ARP请求分组，操作码为2时表示ARP响应分组，操作码为3时表示RARP请求分组，ARP为4时表示RARP响应分组。

​		ARP请求分组用于获取目的IP地址对应的MAC地址，因此目的MAC地址全为0。

​		若主机需要向同一子网下的另一台主机或路由器发送一个IP寻址的数据报，该主机需要获取目的IP地址的目的MAC地址。

​		若主机的ARP表中没有目的IP地址的表项，则需要ARP协议来解析这个IP地址。请求主机构造ARP分组，网络适配器将分组封装成帧时将广播地址作为帧的目的MAC地址。同一子网下的其他网络适配器接收帧后将帧内的ARP分组传递给ARP模块。ARP模块会将分组中的目的IP地址与自身IP地址进行匹配，若匹配则向请求主机发送一个带有所需映射的ARP分组。请求主机根据ARP响应分组更新ARP表。
