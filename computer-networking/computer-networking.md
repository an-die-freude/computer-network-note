...
​		ap3.1加入了加密密码，但这样并不能防止**重放攻击**，攻击者只需窃听通信并记录加密密码，再将重放加密密码。

​		ap4.0加入了类似于TCP建立连接的方式和**不重数**，不重数是协议生存周期中只使用一次的数。被认证实体向认证实体发送认证信息，认证实体返回一个不重数$R$，被认证实体使用共享对称加密密钥$K$来加密$R$并把$K(R)$发回给认证实体。认证实体解密后得到的结果与$R$相等则表示认证成功。
